...
/*
    应用说明：访问串行EEPROM前，请先调用一次
   bsp_InitI2C()函数配置好I2C相关的GPIO.
*/
...
*********************************************************************************************************
*/
int32_t ee_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress,
                     uint16_t _usSize) {
  uint16_t i, m;
...
/*
*********************************************************************************************************
*   函 数 名: ee_CompareBytes
*   功能说明: 从串行EEPROM指定地址处开始比较若干数据
*   形    参:  _usAddress : 起始地址
*            _usSize : 数据长度，单位为字节
*            _pCompareBuf : 带比较的数据的缓冲区指针
*   返 回 值: 0 表示失败，1表示成功
*********************************************************************************************************
*/
int32_t ee_CompareBytes(uint8_t *_pCompareBuf, uint16_t _usAddress,
  ...
  */
  ...
  for (i = 0; i < _usSize; i++) {
    ...
    uint8_t _Value = i2c_ReadByte(); /* 读1个字节 */

    /* 第11步：比较数据异常时返回 */
    if (_Value != _pCompareBuf[i]) {
      i2c_NAck(); /* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
      goto cmd_fail;
    }
