...
*
*               实现了多个软件定时器供主程序使用(精度1ms)， 可以通过修改
*TMR_COUNT 增减定时器个数 实现了ms级别延迟函数（精度1ms） 和us级延迟函数
*               实现了系统运行时间函数（1ms单位）
*
*   修改记录 :
*       版本号  日期        作者     说明
...
*       V1.2    2014-09-07 armfly  增加TIM4 硬件定时中断，实现us级别定时.20us -
*16秒 V1.3    2015-04-06 armfly  增加 bsp_CheckRunTime(int32_t _LastTime)
*用来计算时间差值 V1.4    2015-05-22 armfly  完善 bsp_InitHardTimer()
*，增加条件编译选择TIM2-5 V1.5    2018-11-26 armfly  s_tTmr赋初值0;
*增加g_ucEnableSystickISR变量避免HAL提前打开systick中断 引起的异常。
*
...
/*
*********************************************************************************************************
*   函 数 名: bsp_RunPer100ms
*   功能说明: 该函数每隔10ms被Systick中断调用1次。详见
*bsp_timer.c的定时中断服务程序。一些处理时间要求不严格的
...
*********************************************************************************************************
*   函 数 名: bsp_RunPer1ms
*   功能说明: 该函数每隔1ms被Systick中断调用1次。详见
*bsp_timer.c的定时中断服务程序。一些需要周期性处理的事务
*            可以放在此函数。比如：触摸坐标扫描。
*   形    参: 无
*   返 回 值: 无
*********************************************************************************************************
...
void SysTick_ISR(void) {
  static uint8_t s_count = 0, s_cnt = 0;
  uint8_t i;
  ...
  g_iRunTime++;
  if (g_iRunTime ==
      0x7FFFFFFF) /* 这个变量是 int32_t 类型，最大数为 0x7FFFFFFF */
  {
    g_iRunTime = 0;
  }
  ...
  if (++s_cnt >= 100) {
    ...
    bsp_RunPer100ms(); /* 每隔100ms调用一次此函数，此函数在 bsp.c */
  }
}
/*
*********************************************************************************************************
...
void bsp_DelayMS(uint32_t n) {
  ...
  while (1) {
    ...
    /*
        等待延迟时间到
        注意：编译器认为 s_ucTimeOutFlag = 0，所以可能优化错误，因此
       s_ucTimeOutFlag 变量必须申明为 volatile
    */
...
#if (MCU_CORE_TYPE == MCU_CORE_STM32) // MCU类型
/*
*********************************************************************************************************
*    函 数 名: bsp_DelayUS
...
*   形    参:     _id     :
*定时器ID，值域【0,TMR_COUNT-1】。用户必须自行维护定时器ID，以避免定时器ID冲突。
*               _period : 定时周期，单位1ms
*   返 回 值: 无
*********************************************************************************************************
*/
